      ...
      if (storedToken) {
        try {
          ...
          if (response.ok) {
            ...
            const newCompletedTasks = tasksData.filter(task => task.percentage_done === 100).length;
            const newInprogressTasks = tasksData.filter(task => (task.percentage_done > 0 && task.percentage_done < 100)).length;
            const newWaitingTasks = tasksData.filter(task => task.percentage_done === 0).length;
            const newTotalPercentageDone = tasksData.reduce((sum, task) => sum + task.percentage_done, 0);
            const newPercentageDone = tasksData.length > 0 ? newTotalPercentageDone / tasksData.length : 0;
        ...
        <div className="analytics-bar">
            <div className="analytics-label">
                ...
                <div className="analytics-icon">
                    <button onClick={handleReload}><MdOutlineAnalytics size={24} /></button>
                    <div class="comment-box">Reload</div>
                </div>
            </div>
